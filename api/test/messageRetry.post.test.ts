...
describe("MessageRetry endpoint", () => {
  ...
  async function simulateRequest(
    ...
    const req = new Request(
      `http://localhost/api/v1/messages/${messageUuid}/retry`,
      {
        method: "POST",
        headers: {
  ...
  it("retries a message and sets status to pending", async () => {
    ...
    // Verify pending message attempt was recorded
    const attempt = await db
      .prepare(
        `SELECT status FROM message_attempt WHERE message_uuid = ? ORDER BY attempted_at DESC`,
      )
      .bind("msg-1")
      .first();
    expect(attempt).toEqual({ status: "pending" });
  });
  ...
  it("rate limits the retry if over segment limit", async () => {
    ...
    expect(res.status).toBe(200);
    // Verify rated limited message attempt was recorded
    const attempt = await db
      .prepare(
        `SELECT status FROM message_attempt WHERE message_uuid = ? ORDER BY attempted_at DESC`,
      )
